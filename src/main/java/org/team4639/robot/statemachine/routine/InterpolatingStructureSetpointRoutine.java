...
public class InterpolatingStructureSetpointRoutine {
  ...
  private static String getMapEntry(
      String mapName, double key, SuperstructureState superstructureState) {
    return mapName + ".put(" + key + ", " + getStateConstructor(superstructureState) + ")";
  }

  public static void _registerEntry(
      int level, double distance, SuperstructureState superstructureState) {
    entries.add(getMapEntry("L" + level + "Map", distance, superstructureState));
  }

  public static InstantCommand registerEntry(
      int level, double distance, SuperstructureState superstructureState) {
